...
net.all = read.csv("../2.DX2013_145sites/data/All.plant188.micro.soil.1314.cohesion.csv",
                   row.names = 1)

net.me = melt(net.all, id = c("Elevation", "pre.AI"))

write.csv(net.me, "../2.DX2013_145sites/data/Long.table.all.plant188.micro.soil.1314.cohesion.csv")

coh.last = read.csv("../2.DX2013_145sites/data/Long.table.all.plant188.micro.soil.1314.cohesion.add.AI.csv",
                    row.names = 1)
...
for (i in 1:length(gp)){
  gp.tmp <- gp[i]
  gp.t <- net[which(coh.last$gp == gp.tmp),]
  gp.t = data.frame(gp.t)
  
  AI.t <- AI[which(coh.last$gp == gp.tmp),]
...
write.csv(lm.rich.all, "../2.DX2013_145sites/data/lm.p.value.plant188.soil.micor.1314.coh.csv")
...
ggplot(coh.last, aes(x=pre.AI, y=value, color = gp))+ 
  geom_point(size = 2)+
...
ggsave("../2.DX2013_145sites/figures/AI.plant.cov188.soil.micro.1314.net.cohesion.1314year.last.pdf",
       width = 4.5, height = 5)
