...
rownames(Sp.ra) = rownames(comm.tmp)

colnames(Sp.ra)[1] = c("RA")

# write.csv(Sp.ra, "../1.DX2013_145sites/data/last/8.1.otu.relative.abandance.all.csv")

# decreasing order 
#Sp.ra.ord = arrange(Sp.ra, -RA)
#Sp = data.frame(Sp.ra[order(Sp.ra[, 1]), ])

# Sp.ra.ord = read.csv("../1.DX2013_145sites/data/last/8.1.otu.relative.abandance.all.order.csv",
#                      row.names = 1)
...
if(use.custom.cors == F) {
  if(tax.shuffle) { ###??�or(which.taxon in 1:dim(rel.d)[2]){
...
library(WGsave.image(output, "../2.DX2013_145sites/data/micro1314.network.cohesion.Rdata")

write.csv(output$`Negative Cohesion`, "../2.DX2013_145sites/data/micro1314.Negative.cohesion.csv")
write.csv(output$`Positive Cohesion`, "../2.DX2013_145sites/data/micro1314.Positive.cohesion.csv")
