...
PV.all = read.csv("../2.DX2013_145sites/data/PV.random9sample.all.components.repli4times.deal.with.PLFA.plant.4300m.true.form.dot.plot.csv")
...
PV <- as.data.frame(PV.all[,c("value")])
colnames(PV) <- c("PV")

AI <- as.data.frame(PV.all[,c("Aridity")])


for (i in 1:length(gp)){
  gp.tmp <- gp[i]
  gp.t <- PV[which(PV.all$gp == gp.tmp),]
...
write.csv(lm.rich.all, "../2.DX2013_145sites/data/lm.p.value.scatter.dot.AGB.PLFA.EF.rich.soil.PV.csv")

unique(PV.all$variable)
var.list = c( "Plant.rich", "Micro.rich", "EMF",
              "AGB", "PLFA", "Soil.pca1")
PV.all$variable=factor(PV.all$variable,levels = var.list)
PV.all=PV.all[order(PV.all$variable),]
...
ggplot(PV.all, aes(x=Aridity, y=value, color = Elevation))+ 
...
ggsave("../2.DX2013_145sites/figures/sactter.dot.plot.AGB.PLFA.EF.rich.soil.PV.pdf",
       width = 6.5, height = 5)
