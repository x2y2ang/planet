...
mass.num = data.frame(mass.num)


veg.mass = cbind(mass.all$site, mass.num)

names(veg.mass) = c("", "Veg.mass")

rownames(veg.mass) = veg.mass[,1]

cli175 = cli1314.last[rownames(veg.mass),]
envi175 = envi1314[rownames(veg.mass),]

# delete 10km
# envi175.tmp = subset(envi175, Distance_origin != 10000)
# cli175.tmp = cli175[rownames(envi175.tmp),]
# veg.mass.tmp = veg.mass[rownames(envi175.tmp),]
# 
# mass.AI = cbind(cli175.tmp[,c("Act.ele", "Elevation", 
#                           "pre.bio12", "pre.AI")], veg.mass.tmp$Veg.mass)
# 
# colnames(mass.AI)[5] = c("Veg.mass")


mass.AI = cbind(cli175[,c("Act.ele", "Elevation",
                          "pre.bio12", "pre.AI")], veg.mass$Veg.mass)

colnames(mass.AI)[5] = c("Veg.mass")


# delete 4300 4400 
vec.tmp.de = subset(mass.AI, Elevation != 4300)

vec43 = subset(mass.AI, Elevation == 4300)

write.csv(vec43, "../2.DX2013_145sites/data/plant.biomass.4300.diversity.csv")

Veg.mass.tmp = data.frame(vec.tmp.de[, "Veg.mass"])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  plant.t <- Veg.mass.tmp[which(vec.tmp.de$Elevation == ele.tmp),]
...
write.csv(hori.all, "../2.DX2013_145sites/data/plant.biomass.1314.CV.Asy.elevation.csv")
...
ggplot(hori.all, aes(x=Ele, y=value))+ 
  ...
  xlab("") + ylab("Plant biomass") + # Set axis labels
  theme_set(theme_bw())+
...
ggsave("../2.DX2013_145sites/figures/plant1314.biomass.CV.9site.pdf",
       width = 6.5, height = 3)
...
write.csv(prec.hori.all, "../2.DX2013_145sites/data/plant.biomass.1314.CV.Asy.precipitation.csv")
...
ggsave("../2.DX2013_145sites/figures/prec.plant1314.plant.biomass.CV.9site.pdf",
       width = 6.5, height = 3)


AI.CV.Asy2 = CV.Asy[, c("pre.AI", "plant.CV", "Asy")]
...
write.csv(AI.hori.all, "../2.DX2013_145sites/data/plant.biomass.1314.CV.Asy.aridity.csv")
...
ggplot(AI.hori.all, aes(x=pre.AI, y=value))+ 
  ...
  facet_wrap( ~ variable, scales = "free", ncol = 2)+ 
  xlab("Aridity") + ylab("Plant biomass") + # Set axis labels
  theme_set(theme_bw())+
  theme(panel.grid.major=element_line(colour=NA),
        axis.text.x = element_text(angle = 45, hjust = 0.5, vjust = 0.5),
        panel.grid=element_blank()) +
...
ggsave("../2.DX2013_145sites/figures/Aridity.plant1314.biomass.Asy.9site.pdf",
       width = 6.5, height = 3)
