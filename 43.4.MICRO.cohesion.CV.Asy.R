...
coh.all = read.csv("../2.DX2013_145sites/data/micro.cohesion.last1314.csv",
                   row.names = 1)

rownames(coh.all) = rownames(cli1314.last)
...
#### loop 
net.tmp = data.frame(coh.all[, "total.coh"])
Act.ele = data.frame(coh.all[, c("Act.ele")])
pre.AI = data.frame(coh.all[, c("pre.AI")])


ele = unique(coh.all[,"Elevation"])


i=1
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  net.t <- net.tmp[which(coh.all$Elevation == ele.tmp),]
  ...
  ######## for mean ele #######
  act.ele.t <- Act.ele[which(coh.all$Elevation == ele.tmp),]
  ...
  ####### for mean pre.AI ########
  pre.AI.t <- pre.AI[which(coh.all$Elevation == ele.tmp),]
...
write.csv(hori.all, "../2.DX2013_145sites/data/Total.cohesion.microbial.1314.CV.Asy.elevation.csv")
...
ggsave("../2.DX2013_145sites/figures/Total.cohesion.micro.1314.CV.Asy.pdf",
       width = 6.5, height = 3)


AI.CV.Asy2 = CV.Asy[, c("pre.AI", "micro.net.CV", "micro.net.Asy")]
...
write.csv(AI.hori.all, "../2.DX2013_145sites/data/Total.cohesion.micro.1314.CV.Asy.aridity.csv")
...
ggsave("../2.DX2013_145sites/figures/Total.cohesion.micro.1314.CV.Asy.aridity.pdf",
       width = 6.5, height = 3)
