...
################    loop for 10 eles    ######################
vec.tmp = cbind(cli1314.last, pca.dd[, 1], envi1314)

colnames(vec.tmp)[11] = c("Soil.pca1")

Soil.pca1.tmp = data.frame(vec.tmp[, "Soil.pca1"])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  soil.t <- Soil.pca1.tmp[which(vec.tmp$Elevation == ele.tmp),]
  
  soil.com <- t(combn(soil.t,9))
  soil.com <- data.frame(soil.com)
  rownames(soil.com)
  
  soil.mean = apply(soil.com, 1, mean)
  soil.sd = apply(soil.com, 1, sd)
  
  CV.tmp= soil.sd/soil.mean
  
  # Asy strong functions for row
  soil.max = apply(soil.com, 1, max)
  soil.min = apply(soil.com, 1, min)
  
  Asy = ((soil.max - soil.mean)/(soil.mean - soil.min))
...
CV.Asy = cbind(round(act.ele.CV$Ele, 0), round(act.ele.CV$pre.bio12, 0),
               round(act.ele.CV$pre.AI, 3), act.ele.CV[, c("soil.CV", "Asy")])
...
write.csv(hori.all, "../2.DX2013_145sites/data/soil.pca1.1314.CV.Asy.elevation.csv")
...
ggsave("../2.DX2013_145sites/figures/Fig.69.soil1314.rich.CV.9site.pdf",
       width = 6.5, height = 3)
...
write.csv(prec.hori.all, "../2.DX2013_145sites/data/soil.rich.1314.CV.Asy.precipitation.csv")
...
ggsave("../2.DX2013_145sites/figures/Fig.70.prec.soil1314.rich.CV.9site.pdf",
       width = 6.5, height = 3)


AI.CV.Asy2 = CV.Asy[, c("pre.AI", "soil.CV", "Asy")]
...
write.csv(AI.hori.all, "../2.DX2013_145sites/data/soil.rich.1314.CV.Asy.aridity.csv")
...
pdf("../2.DX2013_145sites/figures/Fig.71.Aridity.soil1314.rich.Asy.9site.pdf",
    width = 6.5, height = 3)
