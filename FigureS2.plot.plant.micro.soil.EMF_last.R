...
gp = c(rep("A", 75), rep("B", 70), rep("A", 42), rep("B", 36))

micro.rich.me1 = cbind(gp, micro.rich.me)

A = subset(micro.rich.me1, gp=="A")
A1 = subset(micro.rich.me1, Elevation!="4700")
summary(lm(A1$value ~ A1$pre.AI, A1))
# Adjusted R-squared:  -0.00329, p-value: 0.4328

B = subset(micro.rich.me1, gp=="B")

summary(lm(B$value ~ B$pre.AI, B))
#Adjusted R-squared:  0.003185, p-value: 0.2505
