...
veg188.cli.tmp1 = subset(veg188.cli.tmp, X < 10000)

Plant.micro.soil.EMF = read.csv("../2.DX2013_145sites/data/DX1314.Plant.micro.soil.EMF.AI.for.PV.csv",
                                row.names = 1)
Plant.micro.soil.EMF1 = subset(Plant.micro.soil.EMF, X < 10000)
...
plant.rich.tmp = data.frame(veg188.cli.tmp1[, "Plant.rich"])
Act.ele = data.frame(veg188.cli.tmp1[, c("pre.AI")])

ele = unique(veg188.cli.tmp1[,"Elevation"])


i=1
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- plant.rich.tmp[which(veg188.cli.tmp1$Elevation == ele.tmp),]
  ...
  # for mean ele
  act.ele.t <- Act.ele[which(veg188.cli.tmp1$Elevation == ele.tmp),]
...
write.csv(plant.rich.PV, "../2.DX2013_145sites/data/66.de10km.2.PV.Test.plant.rich.csv")



############################ micro.rich ################################

Micro.rich.tmp = data.frame(Plant.micro.soil.EMF1[, "Micro.rich"])
Act.ele = data.frame(Plant.micro.soil.EMF1[, c("pre.AI")])

ele = unique(Plant.micro.soil.EMF1[,"Elevation"])


i=1
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- Micro.rich.tmp[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
  ...
  # for mean ele
  act.ele.t <- Act.ele[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
...
write.csv(Micro.rich.PV, "../2.DX2013_145sites/data/66.de10km.3.PV.Test.Micro.rich.csv")
...
EMF.tmp = data.frame(Plant.micro.soil.EMF1[, "EMF"])
Act.ele = data.frame(Plant.micro.soil.EMF1[, c("pre.AI")])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- EMF.tmp[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
...
write.csv(EMF.PV, "../2.DX2013_145sites/data/66.de10km.4.PV.Test.EMF.csv")
...
AGB.tmp = data.frame(Plant.micro.soil.EMF1[, "AGB"])
Act.ele = data.frame(Plant.micro.soil.EMF1[, c("pre.AI")])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- AGB.tmp[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
...
write.csv(AGB.PV, "../2.DX2013_145sites/data/66.de10km.5.PV.Test.AGB.csv")



############################ PLFA ################################

PLFA.tmp = data.frame(Plant.micro.soil.EMF1[, "PLFA"])
Act.ele = data.frame(Plant.micro.soil.EMF1[, c("pre.AI")])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- PLFA.tmp[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
...
write.csv(PLFA.PV, "../2.DX2013_145sites/data/66.de10km.6.PV.Test.PLFA.csv")



############################ Soil.pca1 ################################

Soil.pca1.tmp = data.frame(Plant.micro.soil.EMF1[, "Soil.pca1"])
Act.ele = data.frame(Plant.micro.soil.EMF1[, c("pre.AI")])
...
for (i in 1:length(ele)){
  ele.tmp <- ele[i]
  rich.t <- Soil.pca1.tmp[which(Plant.micro.soil.EMF1$Elevation == ele.tmp),]
...
write.csv(Soil.pca1.PV, "../2.DX2013_145sites/data/66.de10km.7.PV.Test.Soil.pca1.csv")
