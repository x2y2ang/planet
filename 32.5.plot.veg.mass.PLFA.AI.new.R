...
summary(lm.veg.rich)
# Adjusted R-squared:  0.1172 p-value:  9.07e-07
...
# Adjusted R-squared:  0.08094  p-value: 0.01418

B = subset(micro.rich.me1[61:223, ])

summary(lm(B$value ~ B$pre.AI, B))
# Adjusted R-squared:  -0.005687  p-value: 0.6091

lm.T1 = micro.rich.me1$gp %in% c("A")
lm.F1 = micro.rich.me1$gp %in% c("B")

micro.rich.me1$Elevation = factor(micro.rich.me1$Elevation)

p3 = ggplot(micro.rich.me1, aes(pre.AI, value, color = Elevation))+
  geom_point(size = 2)+
  # linear lm sig==T
  stat_smooth(aes(colour=gp),method="lm",formula=y~x,size=0.8,
              se = FALSE,data = micro.rich.me1[lm.T1,])+
  # linear lm sig==F
  stat_smooth(aes(colour=gp),method="lm",formula=y~x,size=0.8, lty = 2,
              se = FALSE,data = micro.rich.me1[lm.F1,])+
  
  scale_colour_manual(values = c(brewer.pal(10,"Paired"), "black",
                                 "black"))+
  theme_bw()+  
  theme(legend.background=element_rect(colour="Black",size=0.5))+
  theme(strip.background = element_blank())+
...
qu.micro.mass = lm(micro.mass.me$value ~ micro.mass.me$pre.AI + I((micro.mass.me$pre.AI)^2), veg.mass.me)
summary(qu.micro.mass)
# Adjusted R-squared:  0.5948 p-value: < 2.2e-16

AIC(lm.micro.mass, qu.micro.mass)
# qu.micro.mass  4 2165.808
...
summary(qu.EMF.AI)
# Adjusted R-squared: 0.7124   p-value: < 2.2e-16

AIC(lm.EMF.AI, qu.EMF.AI)
# qu.EMF.AI  4 227.7912
...
ggsave("../2.DX2013_145sites/figures/AI.PLFA.EMF.1314year.mass.rich.soil.pdf",
       width = 10, height = 5)
