...
net.all = read.csv("../2.DX2013_145sites/data/plant1314.188sites.coverage.cohesion.all.melt.gp.csv",
                   row.names = 1)
...
write.csv(lm.rich.all, "../2.DX2013_145sites/data/lm.p.value.plant1314.coh.csv")
...
se = as.numeric(net.all$se)



ggplot(net.all, aes(x=pre.AI, y=value, color = gp))+ 
  geom_point(size = 2)+
...
ggplot(net.all, aes(x=pre.AI, y=value, color = Elevation))+ 
  ...
  # scale_colour_manual(values = rep(c("#A6CEE3", "#1F78B4"), 10))+
...
ggsave("../2.DX2013_145sites/figures/change.color.AI.plant1314.cov.net.cohesion.1314year.last.pdf",
       width = 4.5, height = 5)
